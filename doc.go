...
var subApiList map[string]string
...
var containModels map[string][]string
	...
	if f.Comments != nil {
		for _, c := range f.Comments {
			for _, s := range strings.Split(c.Text(), "\n") {
				...
				} else if strings.HasPrefix(s, "@SubApi") { // parse SubApi
					re := regexp.MustCompile(`([^\[]+)\[{1}([\w\_\-/]+)`)
					if matches := re.FindStringSubmatch(strings.TrimSpace(s[len("@SubApi"):])); len(matches) == 3 {
						subApiList[matches[2]] = matches[1]
					}
				}
			}
		}
	...
	if comments != nil && comments.List != nil {
		for _, c := range comments.List {
			...
			if strings.HasPrefix(t, "@router") {
				...
				routerPath = e1[0]
				// get tags
				for subApi, subApiDesc := range subApiList {
					if strings.HasPrefix(routerPath, subApi) {
						opts.Tags = []string{subApiDesc}
						break
					}
				}
	...
	for _, pkg := range astPkgs {
		for _, fl := range pkg.Files {
			for k, d := range fl.Scope.Objects {
				if d.Kind == ast.Typ {
					if k == objectname && packageName == pkg.Name {
		...
		for _, field := range st.Fields.List {
			...
			*realTypes = append(*realTypes, realType)
			...
			if field.Names != nil {
				...
				preType := packageName + "." + k
				if StringInSlice(containModels[preType], realType) == -1 {
					containModels[preType] = append(containModels[preType], realType)
				}
...
// handle contains model
func HandleContainModels() {
	for m, mArr := range containModels {
		for mPath1, mSchema1 := range rootapi.Definitions {
			if mPath1 == m {
				for mPath2, mSchema2 := range rootapi.Definitions {
					if StringInSlice(mArr, mPath2) != -1 {
						for k, v := range mSchema2.Properties {
							if _, ok := mSchema1.Properties[k]; !ok {
								rootapi.Definitions[mPath1].Properties[k] = v
							}
						}
					}
				}
